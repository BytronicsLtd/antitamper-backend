...
const controller = {
    /**
     * Register PDA - Called when PDA first connects to a BT device
     * Flow:
     * 1. Check if BT MAC is registered
     * 2. If not, log attempt and return error
     * 3. If yes, get factory from BT device
     * 4. Create/update PDA in staging for that factory
     */
    register: async (req, res) => {
        try {
            const { pda_serial, bluetooth_mac_address } = req.body;

            if (!pda_serial || !bluetooth_mac_address) {
                return res.status(400).send({
                    success: false,
                    message: "pda_serial and bluetooth_mac_address are required"
                });
            }

            // Check if BT device is registered
            ...
            // Get factory details
            const factory = await FactoryModel.findById(device.factory);
            if (!factory) {
                return res.status(404).send({
                    success: false,
                    message: "Factory not found for this device"
                });
            }
            ...
            if (pda) {
                // PDA exists - check if same factory
                if (pda.factory && pda.factory.toString() === factory._id.toString()) {
                    // Same factory - return current status
                    ...
                    // Different factory - put in staging for new factory, revoke old key
                    ...
                    return res.status(200).send({
                        ...
                        factory_id: factory._id,
                        factory_name: factory.name,
                        message: 'PDA moved to staging for new factory'
                    });
                }
            }
            ...
            return res.status(201).send({
                ...
                factory_id: factory._id,
                factory_name: factory.name,
                message: 'PDA registered and pending approval'
            });
